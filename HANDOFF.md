...
- Static `IO` class added for saving/loading model parameters
...
- Removed dead code: `save_hyperparameters` and `load_hyperparameters` methods from `IO` class
- Model save timestamps now ISO 8601 compliant: `%Y-%m-%dT%H-%M-%S`

### Current file layout
```
src/
├── app.py             # Flask server (GET /, POST /predict)
...
templates/
└── index.html         # Drawing interface (canvas, buttons, result display, inline CSS + JS)
```
...
Chosen over Gradio/Streamlit to learn the web stack (HTML, CSS, JavaScript, HTTP, Flask). **In progress.**
...
5. **Image processing** — using Pillow to convert RGBA → grayscale, resize 560×560 → 28×28, normalize, invert for model input

### Files to create
- **`templates/index.html`** — DONE. Drawing interface with 560×560 canvas, two-panel layout, clear/predict buttons
- **`src/app.py`** — DONE (scaffold). Model loading complete. `POST /predict` route pending.

### Pipeline
Canvas (560×560 RGBA, light background) → Pillow resize to 28×28 → grayscale → normalize `/ 255` → invert `255 - pixels` → reshape to `(1, 784)` → `Network.forward_propagation()` → `np.argmax()` → return JSON `{ digit, confidence }`
...
### Canvas UX
Light background (#eee), dark pen (user-friendly). Colors inverted in `app.py` before sending to model (MNIST expects white on black).

---

**Last commit**: `feat: scaffold Flask web frontend for digit prediction`
